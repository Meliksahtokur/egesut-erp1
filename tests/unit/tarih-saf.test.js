...
test('tarihMaskeUygula: yıl 4 haneyi aşamaz — fazlası yutulur + işaretlenir', () => {
  const r = tarihMaskeUygula('05.12.20265');
  assert.strictEqual(r.metin, '05.12.2026');
...
test('tarihMaskeUygula: silme doğaldır — kuyruk ayraç korunur, geri alınmaz', () => {
  ...
  assert.strictEqual(tarihMaskeUygula('11122026').metin, '11.12.2026');
  // çöp non-digit girdi temizlenir
  assert.strictEqual(tarihMaskeUygula('ab?!12cd2026').metin, '12.20.26');
});
...
test('R1 el girişi: 90,09,2026 Uygula → satır içi hata + seçim korunur + yazdığı saklanır', () => {
  ...
  assert.ok(kutu.innerHTML.includes('Eylül 2026 30 gün — 90.09.2026 yok'), 'hata satır içi ve somut: ' + (kutu.innerHTML.match(/giris-hata[\s\S]{0,160}/) || [''])[0]);
});
