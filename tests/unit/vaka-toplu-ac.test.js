...
describe('V2.3 (W18) — 📂 Şablon Yükle kablolaması + ?v= damgası (manifest-içi metin denetimi)', () => {
  ...
  it('index.html: 📂 çipi + yükle alanı tek örnekte; her yerel script ?v=20260926-08 damgalı', () => {
    ...
    const damgasiz = yerel.filter(s => !/\?v=20260926-08$/.test(s));
...
describe('V2.3 (W21) — m-bulk-case tarih alanı yapısı + takvim aksiyonu + manifest damgası', () => {
  ...
  it('manifest link de damgalı: manifest.json?v=20260926-08', () => {
    const html = fs.readFileSync('index.html', 'utf8');
    assert.ok(html.includes('manifest.json?v=20260926-08'), 'manifest damgası 20260926-08');
