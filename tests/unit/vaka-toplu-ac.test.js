...
describe('V2.3 (W18) — 📂 Şablon Yükle kablolaması + ?v= damgası (manifest-içi metin denetimi)', () => {
  ...
  it('index.html: 📂 çipi + yükle alanı tek örnekte; her yerel script ?v=20260913-15 damgalı', () => {
    ...
    // 20260913-15: surum-gecmisi F3 — degisiklikler modülü + api/ui dokunuşu; TÜM damgalar tek değere çekildi
    const srcs = [...html.matchAll(/<script src="([^"]+)"/g)].map(m => m[1]);
    const yerel = srcs.filter(s => !s.startsWith('http'));
    assert.ok(yerel.length >= 14, 'yerel script sayısı: ' + yerel.length);
    const damgasiz = yerel.filter(s => !/\?v=20260913-15$/.test(s));
...
describe('V2.3 (W21) — m-bulk-case tarih alanı yapısı + takvim aksiyonu + manifest damgası', () => {
  ...
  it('manifest link de damgalı: manifest.json?v=20260913-15', () => {
    const html = fs.readFileSync('index.html', 'utf8');
    assert.ok(html.includes('manifest.json?v=20260913-15'), 'manifest damgası 20260913-15');
    for (const eski of ['20260911-14','20260911-13','20260909-12','20260909-11','20260909-10','20260909-9','20260909-8','20260909-7','20260909-6','20260909-5','20260909-4','20260909-3','20260909-2','20260909-1','20260908-1','20260907-4']) {
