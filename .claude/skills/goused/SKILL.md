...
> **⚠️ Bu Legion PC'de GEÇERSİZ (2026-07-04, netleştirildi 2026-07-06):** bu dosyadaki
> `goused-proxy`/`goused-api`/`goused-telsiz` binary'leri (`/root/tools-bank/bin/*`, portlar
> 8742-8744) **başka bir fiziksel makineye (tablet/Termux) özgü**, bu Legion PC'de hiç kurulu
> değil — `goose_start`/`agent_register`/`agent_send`/`agent_receive` MCP tool'ları bu ortamda
> `{"error": "unavailable"}` döner. **Karıştırma:** Goose'un kendisi (`~/.local/bin/goose`
> v1.41.0) bu makinede AYRICA kurulu ve çalışıyor — aşağıdaki telsiz sistemiyle hiçbir ilgisi
> yok. O gerçek çalışan yol için `.claude/skills/goose-master/SKILL.md` (global,
> `scripts/goose-teammate.py` ACP köprüsü). Başka bir ortamda goused-api/telsiz hâlâ
> kullanılıyorsa aşağıdaki içerik geçerlidir.

Tam dokümantasyon: `tools-bank/docs/USAGE_GUIDE.md` § 8 (tools-bank clone'unun bulunduğu yerde)
