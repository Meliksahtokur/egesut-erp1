...
### Kural 5 — Multi-tenancy (Faz 2 hazırlığı)

Sistem **tek-tenant** (RLS `USING(true)`), **multi-tenant Faz 2'ye planlı**. Bu adımda yapılacak tek şey: **yeni** yazdığın tablo / fonksiyon / RPC'ye **farm_id ileri-disiplinini** uygula.

| Durum | Davranış |
|---|---|
| Yeni tenant-scoped tablo | `farm_id uuid NOT NULL DEFAULT '400b9107-a85e-4126-af2c-fd7fe73fb68e'` kolonu + `(farm_id, ...)` index |
| Yeni yazma RPC (tenant tabloya INSERT) | `farm_id = public.current_farm_id()` damgası |
| Yeni RLS policy | `USING(true)` KALSIN — Faz 2'de flip edilecek |
| Global katalog / sistem tablosu | `farm_id` ALMAZ (bkz `.claude/farm-id-discipline.md` §4) |

**Sabit:** `REAL_FARM_ID = 400b9107-a85e-4126-af2c-fd7fe73fb68e` · **Helper:** `public.current_farm_id()` (STABLE, Faz 2'de JWT okur). **Kural gövdesi:** `.claude/farm-id-discipline.md` — burada tekrar etme, link ver.
...
> **Güncel durum (2026-06-13, commit a2e6d00):** `99999999999999_ground_truth.sql` canlı DB ile birebir eşleşiyor:
> - **33/33 egesut tablo** (buzagi_takip orphan silindi, 3 yeni eklendi: tedavi, hayvan_override, vethek_tohumlamalar)
> - **138/138 fonksiyon** (19 eski default'lu imza silindi, 35 yeni default'suz eklendi, 5 duplicate temizlendi)
> - **13/12 view** (1 yeni hayvan_durum_analizi eklendi, 5 hayvan_durum_view + 1 treatment_timeline duplicate silindi)
>
> Regen yöntemi: memory'de `ground_truth_regen_method.md` ve `mcp_supabase_migrate_patterns.md`.
