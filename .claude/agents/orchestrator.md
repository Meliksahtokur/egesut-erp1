...
## FARINDALıK — farm_id ileri-disiplini (Faz 2 hazırlığı)

Sistem KASITLI tek-tenant (RLS `USING(true)`), multi-tenant Faz 2'ye planlı (`docs/superpowers/specs/2026-06-14-login-auth-gate-design.md`). **YENİ** tablo / yazma RPC'si geldiğinde:

- `erp-implementer`'a iletirken: "farm_id ileri-disiplini uygula" notu ekle.
- Tenant-scoped tabloda `farm_id` kolonu + yazma fonksiyonunda `public.current_farm_id()` damgası yoksa implementer'a geri gönder.
- `erp-qa-git` zaten migration validasyonunda kontrol ediyor.

Kural gövdesi: `.claude/farm-id-discipline.md` (kanonik, DRY). SABİT: `REAL_FARM_ID = 400b9107-a85e-4126-af2c-fd7fe73fb68e` · HELPER: `public.current_farm_id()` (STABLE SQL).
