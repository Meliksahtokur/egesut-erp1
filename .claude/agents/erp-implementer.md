...
## farm_id İleri-Disiplini (YENİ nesneler için MUST)

Yazdığın her **yeni tablo** ve **yeni yazma RPC'si** aşağıdaki kontrolden geçer. Detay: `.claude/farm-id-discipline.md`.

```sql
-- YENİ tenant-scoped tablo:
CREATE TABLE public.<yeni> (
  ...
  farm_id uuid NOT NULL DEFAULT '400b9107-a85e-4126-af2c-fd7fe73fb68e'
);
CREATE INDEX ... ON public.<yeni> (farm_id, ...);

-- YENİ yazma RPC (tenant tabloya INSERT):
INSERT INTO public.<tenant_tablo> (..., farm_id, ...)
VALUES (..., public.current_farm_id(), ...);

-- RLS: USING(true) KALSIN. Faz 2'de flip edilecek.
```

İstisna (farm_id ALMAZ): global katalog (`drug_classes`, `diseases`, `irk_esik`), sistem tabloları (`entity_graph`, `memory_notes`, `goose_embeddings`, `agent_*`, `chat_*`), zaten kullanıcı-scoped olanlar. Emin değilsen varsayılan: **operasyonel sürü verisi = tenant-scoped, katalog = global**.
