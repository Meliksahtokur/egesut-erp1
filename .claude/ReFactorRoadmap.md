...
## İlerleme Durumu (2026-08-31 güncellemesi — docs tutarlılık denetimi + fix oturumu)

| Aşama | Durum |
|-------|-------|
| 1.1 Global State | ⚠️ Kısmen — `state.js`/`AppState` var; `_A/_S/_curPg` yok oldu, ~12 global app.js'te kaldı (eski örnek/satır ref'leri bayat) |
| 1.2 Sabitler → config.js | ✅ Bitti — Tüm sabitler `config.js`'te, hiç tekrar tanım yok |
| 1.3 Yardımcılar → utils/ | ✅ DONE — `helpers.js` + `modal.js`; 2026-08-31: modal.js router-stack + cleanup merkezileşti (B2/B3) |
| 1.4 Autocomplete tekilleştirme | ⚠️ `setupAutocomplete()` helpers.js'te ama **çağıran yok** (dead infra); `acHdeTani/acDisease` silindi; kalan `ac*` fonksiyonu (ui.js): `acHayvan` + `acNav` — ya dönüştürülmeli ya da dead-code temizliği. **2026-09-02 kısmi adım:** sıralama+vurgu ortak sözleşmeye alındı — `srchAdaySirala`/`vurguHtml` (helpers.js, testle kilitli) artık `srchDropdown` + `acHayvan`'ın İKİSİNİN de çekirdeği; tekilleştirmenin kalanı render birleşimi |
| 2. Veri Yönetimi | ✅ DONE (bilinçle değişti) — `insertOffline`/`updateOffline` `a71a5e4`'te bilinçli kaldırıldı (dead code); IndexedDB index'leri var; `rpcOptimistic` çalışıyor. 2.2 backoff yerine 2026-08-31'de syncNow skip-and-continue + dead-letter yapıldı (B17) |
| 3.1 Render hafifletme | ⏸️ BEKLİYOR ama **karar verildi** (2026-09-01 benchmark): N≈27 hayvan → dokunma; N>150 → pagination (sayfa 50); N>500 + akış UX şartı → virtual scroll. Darboğaz innerHTML parse/layout, JS hesabı değil |
| 3.2 Event delegation | ✅ DONE — events.js 5 merkezi listener + handlers.js'te 277 kayıtlı action (docs denetimi doğruladı; ui.js dinamik HTML'indeki onclick bilinçli — modal-router kuralı) |
| 3.3 Modal sınıfları | ⏸️ BEKLİYOR |
| 3.4 Toast kuyruğu | ✅ DONE (2026-09-01, eb4a097): FIFO + tavan 3 + dedupe; imza toast(msg,err) korundu; 13 test kilitli |
| 4. Hata Yönetimi | ✅ DONE — `errorHandler.js` self-contained esc (2026-08-31 WP-9); rpc() err.data taşır (B31) |
| 5. Migration | ✅ DONE — GT kanonik referans; **2026-08-31 not:** GT içindeki bazı fn tanımları canlıdan eski (tohumlama_abort 2p/3p vb.) → GT v5 regen idle kuyruğunda |
| 6. Güvenlik/XSS | ⚠️ Kısmen — esc/escAttr/dataset deseni 2026-08-31'de ana ihlal noktalarına uygulandı (B9); kalan iç noktalar sweep bekliyor; tablo-seviyesi guard trigger'ları eklendi (20260831000003) |
| 7. Performans | ✅ debounce/throttle + pullTables sıralı zincir (B18, 2026-08-31) |
| 8. Test/Kod Kalitesi | ⚠️ Kısmen — ESLint var; JSDoc yalnız 6 yerde; unit 22→**302** test (2026-09-01: vm loader + api/toast/buildRpcParams + 13 E2E stub-backend) |
| 9. Dokümantasyon | ✅ README var; CONTRIBUTING.md yok (README gerekçeyi karşılıyor) |

**Son kontrol:** 2026-08-31 (fix oturumu — bug-hunt/docs/test raporları; commit 690b089..41a36df)
