-- Migration: stat_suru_ozet sessiz sayacına COALESCE restore (TASLAK — DEPLOY EDİLMEDİ)
-- Tarih: 2026-09-02 — idle/sessiz-9999-analiz
-- Sorun: 20260625000030_stat_suru_ozet_readonly.sql gövdesinde sessiz sayacı
--        `e.sessiz_gun >= 55` olarak yazılmıştı (COALESCE düşmüş) → v_eligible'da
--        sessiz_gun NULL olan "hiç kayıt yok" hayvanları saymaz. Liste
--        (sessiz_hayvanlar_listele) COALESCE(...,9999) ile onları DAHİL ettiğinden
--        dashboard'ta kart "18", band "24" — aynı ekranda iki farklı sayı (2026-09-02 canlı ölçüm).
-- Çözüm: 20260614000002_stat_suru_ozet_v3.sql'deki kanıtlanmış ifade geri konur:
--        COALESCE(e.sessiz_gun, 9999) >= 55. Tek satırlık restore; gövde geri kalanı
--        canlıyla (pg_get_functiondef, 2026-09-02 doğrulandı) birebir.
-- Etki: stat.hayvan.sessiz 18 → 24; sessiz_hayvanlar_listele çıktısı ve dashboard bandı
--       zaten 24 → kart=band=modal tutarlı. Yeni anahtar eklenmez, JS değişikliği gerekmez.
--       sessiz_hayvanlar_reconcile (cron, 05:00) bilinçli olarak ham `sessiz_gun >= 55`
--       kullanmaya devam eder: NULL hayvanlara vet-kontrol görevi üretilmez (bkz. idle raporu).
