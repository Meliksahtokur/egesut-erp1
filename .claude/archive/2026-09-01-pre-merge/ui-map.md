...
> ⚠️ **GÜNCEL DEĞİL (2026-08-31):** `js/ui.js` son refactor'lerle ~7.760+ satıra büyüdü; aşağıdaki satır aralıkları eski dönemine ait ve büyük ölçüde kaymış durumda. Fonksiyon adları çoğunlukla hâlâ geçerli; **satır aralıklarına güvenme** — `grep -n "function <ad>" js/ui.js` ile canlı konumu doğrula. Tam yeniden üretim ayrı bir görev.
>
> Sessiz hayvan UI yüzeyleri (güncel): dashboard bandı `_dashBands()` içinde `sessizTop` sıralaması (sentinel-son) + modal `_showSessizList()` (canlı konumu: satır ~936, bb4ea92 ile sentinel-son sıralama eklendi).
