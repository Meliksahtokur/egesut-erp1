...
## 14. Integration and root acceptance

Checkpoint: `post-merge` then `final(publishing=true)`, sequence `4`.

The candidate commit `bdd8274` was reconstructed in a detached linked worktree
and revalidated before integration:

```text
detached worktree suite: 64/64 PASS
detached worktree validate: PASS, three goals, zero findings
docs-update without --role in the detached tree: exit 2 (role attestation arrives via Git)
missing receipt in the detached tree: correctly rejected
.harness/memory/README.md: not ignored in a Git-only checkout
```

Integration proceeded in a separate clean worktree (`tmp/phase3-integration`
branch at main `b42c635`) using `git merge --no-commit --no-ff`; the merge
auto-completed without conflicts and this sequence-4 goal/report
reconciliation was carried into the candidate before the merge commit.

Root acceptance of the complete candidate:

```text
candidate parents: b42c635 and bdd8274
manifest vs candidate paths: PASS, exactly 10/10
harness suite: 64/64 PASS
product unit suite: 440/440 PASS
harness validate: PASS, three goals, zero decisions, zero findings
GitNexus staged detection: low risk, 10 files, 0 affected product symbols or flows
git diff checks: PASS
main and origin/main before integration: b42c635
...
GitHub E2E remains explicitly excluded and was not run, waited for, or used as
acceptance evidence. Product, DB, deploy, legacy-agent cleanup, and hook
replacement remain out of scope. Findings 1-4 from the independent review are
recorded in section 12 with Phase 4/5 dispositions.
...
|---|---|
| Phase 3 goal and report | `UPDATED` |
| Remote publish range (b42c635..merge) | `UPDATED` |
| Docs-update contract, CLI, and tests | `NO_CHANGE_REQUIRED` |
| Shared contract, acceptance, and runtime adapters | `NO_CHANGE_REQUIRED` |
| Design SPEC/PLAN stale status headers | `OUT_OF_SCOPE` — untracked main design artifacts outside the exact Phase 3 manifest; remains the recorded root docs-reconciliation follow-up |
| Durable memory promotion | `NO_CHANGE_REQUIRED` — the role-attestation semantics are already owned by `docs-update.md` and this report; no additional curated memory is warranted |
| Product/domain references | `NO_CHANGE_REQUIRED` |

Final docs verdict: `PASS`.

The goal is accepted as `done`. The merge commit, safe main fast-forward,
GitNexus `--index-only` refresh, and remote push follow while preserving the
accepted candidate and the protected main state.
