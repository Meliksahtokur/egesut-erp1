# EgeSüt ERP — Domain Rules (harness reference)
...
- **Toplu aşı (2026-09-02):** m-task-add checkbox'lı seçim → `asi_toplu_planla` 1 parent + alt görevler
  yaratır (her aşı kendi rezervasyonu). Aynı hayvan+aşı+gün mükerrer plan engellenir (RPC + UI).
  1 yıl içinde aynı hastalığı kapsayan aşı (marka bağımsız, vaccine_diseases) varsa UI uyarısı çıkar
  (hard block YOK — kullanıcı kararı). Parent detayında 'Hepsini Uygula' sıralı asi_planli_tamamla.
