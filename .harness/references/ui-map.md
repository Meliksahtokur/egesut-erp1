...
| `js/gecmis.js:olayGunu`, `js/gecmis.js:_gmGunEntriesFromSources` | TG1 Faz 1 single-day ("tarihe git") pipeline: `olayGunu(sourceKey,row)` is the pure event-day rule (date columns verbatim, timestamptz → Europe/Istanbul day, same `_GM_TZ_ESNEK` discipline as `dateKey` — defter `dateKey`/`eventAt` behavior unchanged). `_gmGunEntriesFromSources` adds the 5 day-only sources (vaccination_log, kizginlik_log, stok_hareket, hayvanlar exit/weaning, protokol_instance) with day-view policies (open-case opening day, pending insemination, all islem types) and the dedup priority table (source table wins over its islem_log mirror; referenced stok moves suppressed). Entries carry `sourceKey` explicitly |
| `js/ui.js:gecmisTariheGitAc`, `js/ui.js:gecmisGunSec` | Day-view entry on `#pg-gecmis`: strip buttons (`gecmis-tarihe-git`/`gecmis-gun-bugun`/`gecmis-gun-dun`/`gecmis-gun-kapat` in `js/utils/handlers.js`) open the canonical picker (`tekTarihTakvimAc`, max=today) or set `_gecmisGun`; selected-day banner is a separate surface — `todayKey` passed to `_gmGroupHtml` stays the REAL today (never the selected day) |
