...
async function submitAddDay(btn) {
  if (!navigator.onLine) { toast('⚠️ İnternet bağlantısı gerekli', true); return; }
  if (!_curCase?.id)     { toast('❌ Vaka bulunamadı', true); return; }
  if (_curCase.status === 'closed') { toast('❌ Kapalı vakaya gün eklenemez', true); return; }
  if (btn) { btn.disabled = true; btn.textContent = 'Ekleniyor…'; }
  try {
    const res = await rpc('add_treatment_day', { p_case_id: _curCase.id });
    if (res?.ok === false) { toast('❌ ' + res.mesaj, true); return; }
    toast('✅ Gün eklendi');
    await renderCaseTimeline(_curCase.id);
  } catch(e) { toast('❌ ' + e.message, true); }
  finally { if (btn) { btn.disabled = false; btn.textContent = '+ Gün Ekle'; } }
}

async function submitAddDrug(btn) {
  if (!navigator.onLine) { toast('⚠️ İnternet bağlantısı gerekli', true); return; }
  if (!_curDayId)        { toast('❌ Gün seçilmedi', true); return; }
  const drugId = v('cd-drug-id').trim();
  const dose   = parseFloat(g('cd-dose')?.value);
  const unit   = v('cd-unit').trim();
  const route  = v('cd-route').trim() || null;
  ...
  if (btn) { btn.disabled = true; btn.textContent = 'Kaydediliyor…'; }
  try {
    const res = await rpc('add_drug_administration', {
      p_day_id:  _curDayId,
      p_drug_id: drugId,
      p_dose:    dose,
      p_unit:    unit,
...
async function submitCloseCase(btn) {
  if (!_curCase?.id) return;
  if (!confirm('Vaka kapatılsın mı? Kapatılan vakaya gün/ilaç eklenemez.')) return;
  if (btn) { btn.disabled = true; btn.textContent = 'Kapatılıyor…'; }
  try {
    const res = await rpc('close_case', { p_case_id: _curCase.id });
    if (res?.ok === false) { toast('❌ ' + res.mesaj, true); return; }
    toast('✅ Vaka kapatıldı');
    _curCase = { ..._curCase, status: 'closed' };
    const gunBtn   = document.getElementById('cd-gun-ekle-btn');
    const kapatBtn = document.getElementById('cd-kapat-btn');
    if (gunBtn)   gunBtn.style.display   = 'none';
    if (kapatBtn) kapatBtn.style.display = 'none';
    const meta = document.getElementById('cd-meta');
    if (meta) {
      const chips = meta.querySelectorAll('span');
      if (chips[0]) { chips[0].style.background = 'rgba(78,154,42,.12)'; chips[0].style.color = 'var(--green)'; chips[0].textContent = 'Kapalı'; }
    }
    await renderCaseTimeline(_curCase.id);
    pullTables(['cases']).then(renderSafe).catch(console.warn);
  } catch(e) { toast('❌ ' + e.message, true); }
