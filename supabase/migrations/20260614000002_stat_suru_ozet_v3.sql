-- stat_suru_ozet v3 — ureme_verimlilik (Duve/Inek x 3 katman) + sperma_pi (tohumlama-basina)
-- v2 blokları (cycle_ozet, sperma_top5, deneme) geri uyum icin korundu, UI artik yenileri gosterir
...
DECLARE
  ...
  v_sperma_pi jsonb;
BEGIN
  -- ── Hayvan demografisi (v2 ile aynı) ──
  ...
  -- ── Cycle-bazlı (v2 ile aynı — geri uyum) ──
  WITH cycles AS (
    SELECT
  ...
  -- ── YENİ: Üreme verimliliği (Düve/İnek × 3 katman) — lifetime, son_donem'den bağımsız ──
  WITH basari AS (   -- başarılı cycle'lar + skor (1/N)
    SELECT v.hayvan_id, v.kategori, 1.0 / NULLIF(v.deneme_sayisi, 0) AS skor
    FROM public.v_ureme_dongusu v
    WHERE v.durum = 'Aktif'
      AND v.sonuc = 'Gebe'
      AND v.deneme_sayisi >= 1
      AND (p_padok IS NULL OR v.padok = p_padok)
  ),
  per_animal AS (
    SELECT hayvan_id, kategori, AVG(skor) AS animal_skor
    FROM basari GROUP BY hayvan_id, kategori
  ),
  ham AS (   -- tohumlama-başına ham (Düve/İnek doğum geçmişine göre)
    SELECT
    ...
    WHERE h.cinsiyet = 'Dişi' AND h.durum = 'Aktif'
      AND (p_padok IS NULL OR h.padok = p_padok)
    GROUP BY 1
  )
    ...
    SELECT
      ...
      jsonb_build_object(
        'ham', jsonb_build_object(
          'tohumlama', COALESCE(hm.tohumlama, 0),
          'gebe',      COALESCE(hm.gebe, 0),
          'bos',       COALESCE(hm.bos, 0),
          'bekliyor',  COALESCE(hm.bekliyor, 0),
  ...
  -- ── YENİ: Sperma performansı tohumlama-başına (winning-straw değil) ──
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'toplam', toplam,
      'gebe',   gebe,
      'oran',   ROUND(100.0 * gebe / NULLIF(toplam, 0), 1)
    ) AS row_j
    FROM (
      SELECT
        LOWER(TRIM(split_part(t.sperma, '|', 1))) AS sp,
  ...
  -- Merge: yeni alanları gebelik objesine ekle
  v_gebelik := COALESCE(v_gebelik, '{}'::jsonb)
    || jsonb_build_object(
         'ureme_verimlilik', COALESCE(v_verim, '{}'::jsonb),
         'sperma_pi',        COALESCE(v_sperma_pi, '[]'::jsonb)
       );
