-- G-20260914-GERI-ALMA-AKISI — W4: motor onarımı (luna L4-01/02/03/04/06)
--
-- Onarım turu sözleşmesi: G-20260914-GERI-ALMA-AKISI.md
--   "Onarım turu sözleşmesi (root kararı 2026-09-14/2)" + "K1 uygulama notu".
--
--   L4-01  l4_rehber bayrağı çağırandan OKUNMAZ. Sunucu, zincir önizlemesi
--          bloklanınca ürettiği sıralı rehberin adımlarını
--          surum_gizli.l4_rehber_adimlari tablosuna YAZAR (sunucu-üretimi
--          yetki kaydı); gevşetme yalnız bu kayıtta adımı bulundurunca ve
--          yalnız satır-seviyesi hedeflerde devreye girer. İstemcinin
--          p_hedef'ine koyduğu herhangi bir 'l4_rehber' anahtarı hiçbir yerde
--          okunmaz. (Alternatif mekanizmalar — GUC köprüsü taşınamaz:
--          onizle ve geri_al ayrı transaction'larda koşar.)
--   L4-02  (a) köprü trigger'ı degisim_txid'i HER ZAMAN txid_current() yazar
--          (istemci değeri ezilir; WHEN NULL koşulu kalkar);
--          (b) tip='GERI_ALINDI' INSERT'i yalnız degisim_geri_al içinden:
--          BEFORE INSERT tetikleyicisi app.geri_alma_aktif GUC'i 'on'
--          değilse RED; (c) authenticated doğrudan islem_log INSERT'i kapanır
--          (canlı ölçüm 2026-09-14: policy service_insert FOR ALL TO public;
--          authenticated tablo-grant INSERT açık; uygulama yalnız SELECT —
--          api.js:417; islem_log_select policy'si korunur; RPC'ler SECURITY
--          DEFINER + postgres bypassrls → etkilenmez).
--   L4-03  zincir kapsamı (K1 YENİ): (a) aynı satırın sonraki değişiklikleri
--          + (b) pg_constraint'ten doğrulanan GERÇEK FK bağımlı alt
--          kayıtlarında planı GERÇEKTEN engelleyen değişiklikler. Kolon-adı
--          hayvan köprüsü KALKAR: aynı hayvanın ilgisiz olayları (aşı, kilo,
--          başka kayıt) zincire girmez. Bağımlılık güvenle belirlenemeyen
--          durumda otomatik zincir yok → sirali_rehber.
--   L4-04  rehber adımları TEKİL hedef + satır seviyesiyle çağrılır; motor
--          tarafında {tablo,pk,txid} hedefi satir seviyesinde gevşetmeyle
--          uyumlu. Rehber hedefinden 'l4_rehber' anahtarı KALDIRILDI (yetki
--          sunucu kaydından gelir, bayrak taşınamaz).
--   L4-06  telafi payload.orijinal_tip = GERÇEK islem_log tipi (degisim_txid
--          köprüsünden; çözülmezse hedef tablo + işlem etiketi; 'I,U' harf
--          kümesi kalktı).
--
-- ADDITIVE dışında kalan tek davranış değişiklikleri yukarıdaki onarım
-- maddeleridir; 4 public RPC imzası değişmedi, alan/satir/islem seviyelerinin
-- katı L2 kuralları işaretsiz yolda AYNEN (k3 regresyonu kanıtlar).
-- Replay-safe: IF NOT EXISTS / OR REPLACE / DROP+CREATE.
--
-- DEMO ONLY until the owner's prod deploy gate.

BEGIN;

-- ── L4-02a: köprü trigger — degisim_txid HER ZAMAN txid_current() ───────────
-- Eski gövde yalnız NULL değeri dolduruyor, eski trigger WHEN
-- (NEW.degisim_txid IS NULL) ile KOŞULSUZ tetiklenmiyordu: authenticated,
-- seçilmiş degisim_txid ile sahte köprü satırı yazabiliyordu (luna L4-02).
...
-- ── L4-02b: GERI_ALINDI kapısı — yalnız degisim_geri_al içinden ─────────────
...
CREATE TRIGGER trg_islem_log_geri_alindi_kapisi
  BEFORE INSERT ON public.islem_log
  FOR EACH ROW
  EXECUTE FUNCTION public._islem_log_geri_alindi_kapisi();

REVOKE ALL ON FUNCTION public._islem_log_geri_alindi_kapisi() FROM PUBLIC, anon, authenticated;

-- ── L4-02c: authenticated doğrudan islem_log INSERT kapanır ─────────────────
-- Canlı gerçek (probe_sema_w4, 2026-09-14): policy service_insert = FOR ALL
-- TO public USING true WITH CHECK true; authenticated tablo-grant INSERT
-- açık; ayrıca islem_log_select (SELECT TO public) policy'si mevcut.
-- Uygulama islem_log'a yalnız SELECT yapar (js/api.js:417); tüm iş yazımları
-- SECURITY DEFINER RPC'lerdedir (owner=postgres, rolbypassrls=true → RLS
-- policy'sinden bağımsız). SELECT yolları korunur; INSERT yalnız definer
-- yolunda kalır.
REVOKE INSERT ON public.islem_log FROM authenticated;

DROP POLICY IF EXISTS service_insert ON public.islem_log;
CREATE POLICY service_insert ON public.islem_log
  FOR INSERT TO service_role
  WITH CHECK (true);

COMMENT ON POLICY service_insert ON public.islem_log IS
  'L4-02 (G-20260914-GERI-ALMA-AKISI W4): was FOR ALL TO public; authenticated direct INSERT closed. Business writes go through SECURITY DEFINER RPCs (postgres bypassrls).';

-- ── L4-01: sunucu-üretimi rehber adımı kaydı (jeton tablosu) ────────────────
-- _l4_zincir sirali_rehber ürettiğinde adımları buraya yazar; gevşetme
-- yetkisi YALNIZ bu kayıttan okunur. 7 günden eski adımlar geçersizdir
-- (üretim anında tembel temizlik). authenticated/anon: hiçbir erişim.
...
REVOKE ALL ON surum_gizli.l4_rehber_adimlari FROM PUBLIC, anon, authenticated;
...
REVOKE ALL ON FUNCTION surum_gizli._l4_rehber_uyesi(jsonb) FROM PUBLIC, anon, authenticated;

-- ── plan: L4-01 gevşetme yetkisi sunucu kaydından (gövde birebir korunur) ──
...
-- ── zincir: L4-03 (K1 YENİ — gerçek FK) + L4-01 jeton üretimi + L4-04 ───────
...
-- ── public RPCs: L4-02b GUC kapısı + L4-06 orijinal_tip ─────────────────────
