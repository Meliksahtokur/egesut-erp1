-- Migration: Repeat breed tespiti için hayvan_durum_view'a yeni CTE
-- Etkiler: repeat_breed CTE + repeat_breed_active/past boolean kolonları
-- Tablolar: sadece view değişikliği (tohumlama okuma)
-- Geri alınabilir: DROP VIEW + eski view'ı geri yükle

BEGIN;

-- tohumlanabilir_hayvanlar hayvan_durum_view'a bağımlı olduğu için CASCADE gerekli
...
-- YENİ: Repeat breed analizi — son 15 gündeki tohumlama sayısı
repeat_breed AS (
  SELECT
    hayvan_id,
