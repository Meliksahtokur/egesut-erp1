-- 20260925000017: start_first_service_protocol fonksiyon adı düzeltmesi (U1, KRİTİK)
-- 000013:160'daki "tedavi_sablon_tohumulama_gorev_ekle" (fazla 'u') hiçbir yerde
-- tanımlı değil; RPC non-skip yolunda "function ... does not exist" ile düşüyordu.
-- Gövde CANLI DEMO'dan (ref vtzqjmazsvurxdeondmi) alınmıştır; bu migration YALNIZ
-- o tek fonksiyon adını düzeltir (tohumulama -> tohumluma), başka hiçbir şeyi
-- değiştirmez. 000013/000016 değişiklikleri aynen korunur.
-- ACL: CREATE OR REPLACE mevcut GRANT/REVOKE durumunu korur (anon EXECUTE yok).
...
-- ACL yeniden beyanı (000013 ile birebir; CREATE OR REPLACE zaten korur —
-- sessiz varsayım kapalı: durum açıkça yazılır). anon/PUBLIC EXECUTE YOK.
...
-- EOF 20260925000017_start_protocol_fonksiyon_adi (U1)
