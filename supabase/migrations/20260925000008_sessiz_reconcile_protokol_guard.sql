-- ============================================================================
-- Migration: 20260925000008_sessiz_reconcile_protokol_guard
-- Tarih: 2026-09-25 · Review onarımı (sonsuz kapat-üret döngüsü)
-- Etkiler: public.sessiz_hayvanlar_reconcile() gövdesi
--          (CREATE OR REPLACE, imza değişmez)
-- Geri alınabilir: evet — önceki gövde 20260625000020'den
--          CREATE OR REPLACE ile geri yazılır
-- ============================================================================
-- HATA: ureme_temizlik_reconcile R1 bacağı, aktif protocol_family vakası olan
-- hayvanın açık SESSIZ görevini iptal=true ile kapatıyordu; ancak
-- sessiz_hayvanlar_reconcile (günlük 05:00 cron) üret guard'ında aktif protokol
-- vakası koşulu olmadığından görevi ertesi gün yeniden üretiyordu → döngü.
-- ONARIM: üret döngüsüne "hayvanın aktif (status='active') protocol_family
-- vakası varsa üretme" bacağı eklenir. KAPAT bacağına dokunulmaz.
-- Anon GRANT yazılmaz (lockdown sonrası şablon kuralı).
...
-- EOF 20260925000008_sessiz_reconcile_protokol_guard
