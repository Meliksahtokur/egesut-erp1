-- Migration: Repeat breed fix — deneme_no bazli tekrar tespiti + count
-- Sorun: migration 03 15-gunluk pencereye donmus, hic hayvan eslesmiyor
-- Cozum: migration 02'deki deneme_no mantigina don + repeat_breed_count ekle
-- Geri al: DROP VIEW + onceki versiyonu geri yukle
...
repeat_breed AS (
  SELECT
    hayvan_id,
    MAX(deneme_no) AS max_deneme_no
  FROM public.tohumlama
  GROUP BY hayvan_id
)
SELECT
  ...
  -- deneme_no bazli repeat breed tespiti (migration 02 mantigi)
  COALESCE(st.toh_deneme_no >= 2 AND st.toh_sonuc = 'Bekliyor', false) AS repeat_breed_active,
  COALESCE(rb.max_deneme_no >= 2 AND rb.max_deneme_no > st.toh_deneme_no, false) AS repeat_breed_past,
  -- repeat_breed_count: maksimum deneme_no'dan kac tekrar yapildigi
  GREATEST(0, COALESCE(rb.max_deneme_no, 1) - 1) AS repeat_breed_count
FROM yas y
