-- AI Asistan Faz 2 — tohumlama_kaydet + padok_toplu + dogum_kaydet step
-- Her iki helper tam gövde yeniden tanımı (7 step tipi).
-- NOT: padok_degistir_toplu {success,error} döndürür (ok değil) → runner'da success kontrol.
--      tohumlama_kaydet 2 overload (6/7 arg) → named notation + p_vwp_override ile disambiguate.
...
BEGIN
  ...
  WHEN 'tohumlama_kaydet' THEN
    ...
    RETURN jsonb_build_object('ok',true,'onizleme',
      (SELECT kupe_no FROM public.hayvanlar WHERE id=p_param->>'hayvan_id')||' → tohumlama: '||(p_param->>'sperma'));
  ...
  WHEN 'dogum_kaydet' THEN
    ...
    RETURN jsonb_build_object('ok',true,'onizleme',
      (SELECT kupe_no FROM public.hayvanlar WHERE id=p_param->>'anne_id')||' doğum → buzağı '||(p_param->>'buzagi_kupe')||
      ' (anne otomatik Sağmal''a alınır)');
...
BEGIN
  ...
  WHEN 'tohumlama_kaydet' THEN
    -- named notation + p_vwp_override → 7-arg overload net seçilir (ambiguity önlenir)
  ...
  WHEN 'padok_toplu' THEN
    ...
    -- padok_degistir_toplu {success,error} döndürür
