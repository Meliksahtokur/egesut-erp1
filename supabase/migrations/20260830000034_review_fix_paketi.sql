-- Fix: review bulguları #7 #8 #9 #10 #11 (2026-08-30)
-- #7  tohumlama_abort: abort_tarihi validasyonu (ileri tarih / tohumlama öncesi)
-- #10 tohumlama_abort: açık gebelik-kontrol görevleri + protokol scaffold iptali
-- #8  tohumlama_kaydet: vwp_override kolonu + VWP_OVERRIDE audit'i anchor branch'e bağlandı
-- #9  legacy abort_kaydet(text,text) REVOKE (abort_tarihi'siz, logsuz bypass)
-- #11 backfill düzeltmesi: trigger-yazması (ref_id NULL) abortların tarihi snapshot'tan
-- Taban: 20260830000010 gövdeleri (canlı ile byte-clean doğrulanmış)
...
-- #9: legacy bypass kapatılır (service_role etkilenmez)
REVOKE EXECUTE ON FUNCTION public.abort_kaydet(text, text) FROM authenticated, anon;

-- #11: trigger-yazması abortların abort_tarihi'sini islem_log snapshot'ından düzelt
UPDATE public.tohumlama t
SET abort_tarihi = COALESCE(t.abort_tarihi, (
  SELECT MIN(il.tarih)::date FROM public.islem_log il
  WHERE il.tip = 'ABORT_KAYDI' AND il.ref_id IS NULL AND il.snapshot->>'id' = t.id::text
))
WHERE t.sonuc = 'Abort' AND t.abort_tarihi IS NULL
  AND EXISTS (SELECT 1 FROM public.islem_log il
              WHERE il.tip = 'ABORT_KAYDI' AND il.ref_id IS NULL AND il.snapshot->>'id' = t.id::text);
