...
- **U-1 (OBSERVED — onarım turunda kapandı):** Açık `OVSYNC_BASLAT` görevlerinde `hedef_saat IS NULL` oranı **0/29** (yazıcı ölçümü) ve **0/31** (çapraz-check, aynı gece — payda canlı veri kayar; F10). Sıfır-iddiası iki ölçümde de ayakta. B5 kozmetiği koşullu-basım kuralıyla her oranda doğru davranır; ölçüm Y2 raporuna bilgi satırı olarak girer. Uygulayıcı 0e'de yeniden ölçer.
- **U-2 (UNKNOWN):** "12 aylık düve" gözleminin kök nedeni (veri hatası mı, eski-kural görevi mi) Y2 raporu olmadan SÖYLENEMEZ — spec bilinçli olarak tahmin üretmez; kabul 12 kapıya bağlar. **Ön-kanıt (OBSERVED 2026-09-24):** onarım turu Y2-tipi ölçümünde `sapma < 0` satır sayısı **0/29** (payda canlı kayar — F10) — gözlemin bugünkü açık görevlerde karşılığı yok; 0e nihai koşumu bunu teyit eder.
...
- **V-5 (OBSERVED 2026-09-24, iki ölçüm — onarım turu eki + çapraz-check düzeltmesi):** Canlı demo RPC penceresi **ZAMANA BAĞLIDIR**. İlk ölçümde "0 satır / en yakın hedef 2026-10-06" okundu (yazıcı); çapraz-check bu okumu DÜZELTTİ: `ovsync_baslat_uyarilari` 2026-09-24 gecesi **1 satır** döner (kupe 32, Düve (Büyük), hedef **2026-09-26** = bugün+2 sınırında, tai 2026-10-06 — "2026-10-06" bu görevin TAI tarihiydi, hedef değil); açık `OVSYNC_BASLAT` görevi **31** (yazıcı ölçümünde 29 — canlı veri kayar). Sonuçlar değişmez: (a) e2e RPC-stub şartı (kabul 15) — gerekçe artık **determinizm** (pencere bazen dolu bazen boş; test canlı veriye bağımlı kalamaz), (b) hedef 2026-09-26 geçilince pencere ~2026-10-04'e (sıradaki hedef 2026-10-06'nın hedef−2'si) kadar boşalır — o aralıkta sahip yürüyüşünde bölüm doğal satır GÖSTERMEZ; teslim anında RPC satır sayısı yeniden ölçülür ve teslim metnine yazılır. Bu bir kusur DEĞİLDİR: sunucu pencere kuralının doğru davranışıdır (D13).
...
| F10 | **F9(c)/V-5 çapraz-check (pencere zaman-bağlıdır):** F9 ölçümünden hemen sonra canlı RPC yeniden çağrıldı → **1 satır** döndü (kupe 32, "Düve (Büyük)", hedef **2026-09-26** = bugün+2 sınırında, tai **2026-10-06** — F9'daki "min hedef 2026-10-06" bu görevin TAI tarihiyle karışmış); açık görev sayımı **31**. Sonuç: V-5 "zamana bağlı pencere" olarak yeniden yazıldı; e2e stub şartı KALDI, gerekçesi "pencere boş" değil **determinizm** oldu; U-1/U-2 paydaları canlı-veri kayar (0/29 → çapraz-check 0/31; sıfır-iddiası iki ölçümde de ayakta). Pencere doluluk-örneği, sahibin yürüyüş zamanlamasına göre değişebileceği için teslim notunda canlı ölçüm şartı korundu | OBSERVED tools-bank supabase: RPC `ovsync_baslat_uyarilari {}` → 1 uyarı; `gorev_log` açık OVSYNC_BASLAT SELECT → 31 satır, min hedef 2026-09-26; `hedef_saat IS NULL` → 0 satır (2026-09-24 gecesi) |
