...
- **U-1 (OBSERVED — onarım turunda kapandı):** Açık `OVSYNC_BASLAT` görevlerinde `hedef_saat IS NULL` oranı **0/29** (DEMO psql ölçümü) ve **0/31** (çapraz-check — tools-bank kanalı = **PROD**; payda kanal-bazlıdır: demo 29 / prod 31 — spec-s1 K12 + F10 kanal-düzeltmesi). Sıfır-iddiası iki ölçümde de ayakta. B5 kozmetiği koşullu-basım kuralıyla her oranda doğru davranır; ölçüm Y2 raporuna bilgi satırı olarak girer. Uygulayıcı 0e'de DEMO kanalından yeniden ölçer.
- **U-2 (UNKNOWN):** "12 aylık düve" gözleminin kök nedeni (veri hatası mı, eski-kural görevi mi) Y2 raporu olmadan SÖYLENEMEZ — spec bilinçli olarak tahmin üretmez; kabul 12 kapıya bağlar. **Ön-kanıt (OBSERVED 2026-09-24):** onarım turu Y2-tipi ölçümünde `sapma < 0` satır sayısı **0/29** (DEMO; PROD çapraz-check'inde 0/31 — kanal-bazlı payda, F10) — gözlemin bugünkü açık görevlerde karşılığı yok; 0e nihai koşumu bunu teyit eder.
...
- **V-5 (OBSERVED 2026-09-24, iki ölçüm — onarım turu eki; kanal ayrıştırması tutarlılık turu düzeltmesi):** RPC penceresi **KANAL+GÜN bağılıdır**. İki ölçüm FARKLI kanallardandı — bu, sonradan netleşti (spec-s1 K12): (i) **DEMO** (psql, sahibin yürüyüşünün kanalı): 0 satır; açık `OVSYNC_BASLAT` 29, en yakın hedef **2026-10-06** → pencere ~2026-10-04'e (hedef−2) kadar BOŞ; (ii) **PROD** (tools-bank çapraz-check — F10): 1 satır (kupe 32, Düve (Büyük), hedef **2026-09-26** = bugün+2 sınırında, tai 2026-10-06); açık görev 31. İlk yazımdaki "0 satır / en yakın hedef 2026-10-06" DEMO okumasıydı ve doğrudu; "1 satır" okuması PROD'undu — "canlı veri kayar" yorumu "kanal farkı (+zaman)" olarak düzeltildi. Sonuçlar değişmez: (a) e2e RPC-stub şartı (kabul 15) — gerekçe **determinizm** (pencere kanala ve güne göre değişir; test canlı veriye bağımlı kalamaz), (b) **DEMO** yürüyüşünde İlk Tohumlama bölümü ~2026-10-04'e kadar doğal satır GÖSTERMEZ; sentetik e2e kanıtı + Y2 raporu teslim zarfında birincil kanıttır; teslim anında **demo** RPC satır sayısı yeniden ölçülür ve teslim metnine yazılır. Bu bir kusur DEĞİLDİR: sunucu pencere kuralının (hedef ≤ bugün+2) iki kanalda da doğru davranışıdır (D13).
...
| F9 | **İkinci dalga (aynı onarım turu, kanıt-tazeleme):** (a) **kabul 15'e e2e RPC-stub şartı** — `injectProtokolUyari` yalnız `window.__protokolUyarilar`'ı besler, `?` rozetinin bölümü `rpc('ovsync_baslat_uyarilari')` çıktısıdır (ui.js:1830-1845); canlı demo penceresi boş olduğundan stub'sız e2e deterministik değildi; (b) **§11 U-1 kapandı** — `hedef_saat IS NULL` 0/29 (OBSERVED); U-2'ye ön-kanıt — `sapma<0` 0/29 (OBSERVED); (c) **V-5 eklendi** — pencere boş: 29 açık görevin en yakın hedefi 2026-10-06, RPC 0 satır → sahip yürüyüşü ~2026-10-04'e kadar doğal satır görmez, e2e stub + Y2 raporu birincil kanıt; (d) §12'ye injectProtokolUyari itirazı; (e) **plan-s4.md tam onarımı**: başlığa v1.1 notu, Adım 0a'ya worktree-analyze kuralı (F7'nin vaadi yerine geçti), Adım 0b'ye 33-çağrı beklentisi, Adım 1 E.2 kapsamı, Adım 2'ye RPC-stub adımı, Adım 3 E.6'ya panel-Başlat stopPropagation iddiası, Adım 4.3'e 33-nokta notu, Adım 4.5'e pencere-boş + U-1 teslim notu, §7'ye pencere-boş engel-değil kaydı, kanıt tablosunda 11-12 güncellendi + 7 yeni satır (13-19) | CONFIRMED: stub şartı modal-router.spec.js:27-37 + ui.js:1830-1845 okuması; ölçümler canlı demo SELECT/RPC 2026-09-24 (psql: 29 açık görev, min hedef 2026-10-06, NULL_SAAT 0, NEG_SAPMA 0, RPC_SATIR 0) — (c)'nin pencere okumu F10'da düzeltildi |
| F10 | **F9(c)/V-5 çapraz-check + kanal ayrıştırması (tutarlılık turu düzeltmesi):** F9'un psql ölçümü **DEMO**ydu (29 açık, min hedef 2026-10-06, RPC 0 satır); hemen sonraki tools-bank RPC çapraz-check'i **PROD**'u okudu → **1 satır** (kupe 32, "Düve (Büyük)", hedef **2026-09-26** = bugün+2 sınırında, tai **2026-10-06**), açık görev **31**. F9'daki "min hedef 2026-10-06" DEMO değeriydi (zamanla da değişir); "1 vs 0 satır" ve "29 vs 31" farklarının asıl nedeni **kanal** farkıdır (spec-s1 K12 ile uyumlu). Sonuç: V-5 "kanal+gün bağılı pencere" olarak yazıldı; e2e stub şartı KALDI, gerekçesi **determinizm**; U-1/U-2 paydaları kanal-bazlı (demo 29 / prod 31; sıfır-iddiası iki kanalda da ayakta). Pencere doluluk-örneği kanala ve güne bağlı değişebileceğinden teslim notunda canlı ölçüm şartı korundu | OBSERVED tools-bank (= PROD): RPC `ovsync_baslat_uyarilari {}` → 1 uyarı; açık OVSYNC_BASLAT SELECT → 31 satır, min hedef 2026-09-26; `hedef_saat IS NULL` → 0 satır (2026-09-24 gecesi) — kanal kimliği: spec-s1 KANAL KURALI (tools-bank ref `zqnexqbdfvbhlxzelzju`) |
...
uygulanmamış (`kisir` ui.js'de 23 eşleşme — etiket/filtre/form yüzeylerinde; **hiçbiri**
`_ovSatir` bölgesinde 1834-1845'te değil).
