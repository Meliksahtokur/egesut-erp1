...
| F3 | `supabase/migrations/20260925000003_cila_t6_sonuc_bos_overload_temizligi.sql` (YENİ, **KOŞULLU** — §5'te koşullar) | T6 | DB kulvarı |
| F4 | `js/api.js` | T5 (yok — api.js dokunulmaz T5'te), T8 (`rpc()` mesaj zenginleştirme :86-90), T7-D17 (`RPC_TABLES.start_first_service_protocol` :321) | JS kulvarı |
| F5 | `js/ui.js` | T3 (ui tarafı `_pgKapiBosAtaUygula` :988-1004), T4 (`flushPendingDone` :585-602), T5 (`buildRpcParams` :9426-9427), T7-O10 (`dataTrafficGonder` :9229-9234), T7-D18 (`_erteleKaydet` :1064-1065), T7-O11 (`_protokolUygulaKaydet` :2338-2340), T8 (ölü dal :997), T10 (`loadDash` rozet bloğu :412-424 + panel önbellek :1835-1854) | JS kulvarı (aynı yazıcı, F4-F7 ile **sıralı**, paralel DEĞİL) |
| F6 | `js/forms.js` | T3 (toast kapısı `seansTamamla` :4012-4014), T7-D19 (kapanış özeti :344-350) | JS kulvarı |
...
- Otomatik sync doğru semantikte (ham REST PATCH, alanlar olduğu gibi): CONFIRMED `js/api.js:546-548` (`dbUpdate(op.table, idMatch[1], op.data[0])`).
- Kuyruğa düşen `gorev_log` PATCH yazmalarının üreticisi offline `write()` yoludur: CONFIRMED `js/api.js:232-240` (`_writePatch`); iptal üreticisi örnek: `js/ui.js:1193` (`ovsyncIptal` → `write('gorev_log',{...iptal:true,...},'PATCH',...)`).
...
- Tek-argümanlı eski overload (text): CONFIRMED `20260512000006:L4` + `:L44` `GRANT EXECUTE ... (text) TO anon, authenticated` (eski anon GRANT — bugünkü kuralın konusu değil, sadece risk kaydı).
- **Canlı overload durumu UNKNOWN:** pg_proc PostgREST'ten açılmadı (PGRST205, OBSERVED); "Boş" vs "Bos" hangi gövdenin yazdığı repodan çözülemez. Migration gövdelerinde ASCII `'Bos'` sonucu en-yeni kanonik gövdede YOK (OBSERVED grep) — eski overload gövdesinde olup olmadığı canlı ölçümle netleşecek.
...
### 5.3 F3 koşullu migration taslağı — YALNIZ şu üç koşulun TAMAMI ölçümle doğrulanırsa yazılır/uygulanır
1. Canlıda `(text)` ve `(text,text)` overload'ları **birlikte** duruyor;
2. `(text)` overload'ının gövdesi kanonikten davranışsal fark taşıyor (ör. ASCII `'Bos'` yazıyor ya da görev-iptal yan etkisi eksik/fazla);
3. Sahip ölçüm raporunu onayladı.

```sql
-- 20260925000003_cila_t6_sonuc_bos_overload_temizligi.sql (TASLAK — koşullu)
DROP FUNCTION IF EXISTS public.tohumlama_sonuc_bos(text);   -- eski overload
-- Kanonik (text,text) dokunulmaz; imza listesi ölçüm raporundakiyle birebir eşleşmeli
```
Koşullar tutmazsa: F3 YAZILMAZ, ölçüm raporu bulgularıyla kapatılır (kod değişikliği 0 — bu da meşru bir teslimdir).

### 5.4 Kabul testleri
1. **Ö1:** Adım 0 ölçüm raporu — imza listesi + 'Boş/Bos' dağılımı + anon ayrıcalık — sahibe sunuldu (demo salt-okunur).
2. **Ö2 (F3 uygulanırsa):** ölçüm sonrası `pg_proc` içinde `tohumlama_sonuc_bos` **tek imza** `(text,text)`; `has_function_privilege('anon', ...)` = false.
3. **Ö3 (F3 uygulanırsa):** demo'da PG kapısı "Boş ata ve uygula" akışı (ui.js:996) çalışır: `tohumlama` satırı `sonuc='Boş'`, akış hatasız (plan-5 §3.2 K5 yürüyüş maddesine bağlanır — koşumu SAHİP yapar).
...
- **T4 (flush op kaybı):** CONFIRMED `js/ui.js:585-602` — `flushPendingDone`: `:587` `_pendingDone.clear()` **işleme döngüsünden ÖNCE**; `:598-600` catch yalnız toast → hatalı op kalıcı kayıp. **Plan-5'teki "PG_KAPI kapısını atlıyor" formülasyonu bu spec'te revize edildi:** kapı atlanmıyor — `rpcSeansTamamla` (api.js:693-701) PG RAISE'de `_pgKapiHata` ile modalı arka-planda açıyor; gerçek sorun clear-önce (:587) yüzünden **Vazgeç'te op'un zaten silinmiş olması**. (CONFIRMED okuma; plan-5 §2.2 T4'ün mekanik tanımı düzeltildi.)
...
2. **F5 (ui.js:585-602) flush yeniden yazımı:** clear döngüden SONRA taşınır; op yalnız **başarılı** işlemde pending'den düşer:
```js
...
```
   `recoverPendingDone` (ui.js:604-612) davranışı korunur — yeniden giriş flush'ı tekrar dener.
3. **F5 (ui.js:988-1004) `_pgKapiBosAtaUygula`:** ölü `if (!r?.ok)` dalı silinir (T8); `tekrar` çağrısı try/catch'e alınır — hata durumunda `_pgKapiKapat()` ÇAĞRILMAZ, modal açık kalır ve "Tohumlama Boş kaydedildi, PG uygulanamadı — tekrar dene" net mesajı + aynı butonla yalnız-PG-retry verilir (yarım durum görünür ve telafi edilebilir olur).
...
5. **D6 (demo):** `rpc()` hata mesajı artık sunucunun Türkçe `'error'` metnini taşıyor (ör. "Sadece Bekliyor durumundaki tohumlama boş ilan edilebilir" toast'ta aynen görünür — `20260924000001:L524` metni).
...
**UNKNOWN (Adım 0 ölçümü şart):** senkron zincir görevlerinin `kaynak`/`aciklama` alanlarında 'senkron' damgasının canlı demo verisinde gerçekten geçmesi — ölçüm:
```sql
SELECT gorev_tipi, kaynak, count(*) FROM public.gorev_log
WHERE gorev_tipi IN ('ILAC','TOHUMLAMA_HAZIRLIK','TEDAVI_GUN')
GROUP BY 1,2 ORDER BY 1,3 DESC LIMIT 30;
```
Damga canlıda yoksa: F2 yazılmaz; bulgu "ENGEL" olarak raporlanır ve S1 ile birlikte sahibe taşınır (alternatif eşleşme alanı ölçümden çıkar).
...
- **F5 tasarımı:** `syncNow()` öncesi sayı+tablo kırılımı gösteren onay adımı: kuyruktan `getQueue()` ile kırılım (`gorev_log × n, tohumlama × m …`) içeren `confirm()` (mevcut UI dili Türkçe; ayrı sheet GEREKMEZ — en küçük doğru çözüm). Tek-kayıt `↑` (ui.js:9240) onaysız kalır.
...
- Kanıt: RPC `toplam_erteleme_gun` döndürüyor CONFIRMED `20260923000003:L620-625`; UI toast'u bu alanı göstermiyor CONFIRMED `js/ui.js:1064-1065`; `GOREV_ERTELENEMEZ`/`GECMIS_TARIH` kodları sözlükte yok → errorHandler jenerik dalına düşüyor (CONFIRMED `js/utils/errorHandler.js` sözlük döngüleri ~:40-62).
...
- Kanıt: rozet yalnız `protokol_eksik_tara` çıktısını sayıyor CONFIRMED `js/ui.js:413-424` (rozet filtresi :417); `protokol_eksik_tara`'da OVSYNC bölümü YOK (bölümler: A doğum-sonrası :25, B ileri gebe :127, C kızgınlık :264 — CONFIRMED `20260718000001:L25,L127,L264` + `RETURN v_result`; repo-level ön-şart okuması plan-5 §2.4'ün istediği doğrulama bu spec'te yapıldı); panelin OVSYNC bölümü ayrı RPC `ovsync_baslat_uyarilari`'ndan besleniyor CONFIRMED `js/ui.js:1829-1855` (ovHtml :1829).
- **Tasarım (b2 — varsayılan, DB değişikliği YOK):** `loadDash` rozet bloğu ikinci RPC'yi de çağırıp tek rozette birleştirir: `rozet = aktifProtokolSayısı + ovUyarıSayısı`; `window.__ovsyncUyarilar` önbelleğine yazar, panel (ui.js:1835) taze kayıt yoksa bu önbelleği kullanır (çift çağrı önlenir; panel açılışında kendi çağrısı her zaman tazeler).
...
| # | İfade | Etiket |
|---|---|---|
| V1 | `gorev_tamamla` canlı imzası `(text,text)` (ground_truth:7486'dan) | **UNKNOWN** — F1 apply öncesi canlı `pg_get_functiondef` ile zorunlu teyit |
| V2 | canlı `gorev_tamamla` gövdesinde islem_log INSERT deseni mevcut (F1 iptal-branı izini ona göre yazar) | **UNKNOWN** — canlı gövde okuması şart |
| V3 | senkron zincir görevlerinde `kaynak`/`aciklama` 'senkron' damgası canlı demo'da geçiyor | **UNKNOWN** — §7.2 ölçümü olmadan F2 yazılmaz |
| V4 | canlıda `tohumlama_sonuc_bos` (text) overload'ı hâlâ duruyor ve davranışsal fark taşıyor | **UNKNOWN** — PGRST205 (OBSERVED); §5.2 ölçümüne bağlı |
...
2. **S1 (T1):** senkron muafiyet kapsamı — her senkron ILAC zinciri (taslak) mi, yalnız Presynch-14 mü?
3. **S3 (T10):** rozet-panel tek veri kaynağına DB geçişi (alternatif a) isteniyor mu — varsayılan (b2) DB'siz birleştirme.
4. **T6-F3 onayı:** §5.3'ün üç koşulu ölçümle doğrulanırsa legacy overload DROP onayı.

## 14. Uygulama sırası ve çıkış kapıları

1. Adım 0 (bu spec'in ölçümleri): §5.2 + §7.2 sorguları (salt-okunur, demo) — T6 ve T1'in UNKNOWN'larını kapatır; 3 kırmızı unit test triage'ı (plan-5 §3.4).
...
4. T1: F2 (ölçüm sonrası) → D7-D8; T2 doğrulama D9 (Plan 2 temizliğinden sonra koşar — bu spec'in sırasını bekler).
5. T6: F3 yalnız koşullar tutarsa → Ö1-Ö3.
...
8. **Son review kapısı** (sahibin talebi): ayrı review turu — bu spec'in tüm kabul maddeleri yeniden ölçülür; BUGS.md borç kalemlerine (T9, erteleme-geneli) yeni borç EKLENMEDİĞİ teyit edilir.
