...
| R9 | **Son review 2. tur onarımı (2026-09-25 — teyit ajanı düzenledi, onarım ajanı kapılarla yeniden-doğruladı):** (a) yeniden-tanımlanan üç SECURITY DEFINER fonksiyona da `SET search_path = public, pg_temp` pin'i — R6(a) yalnız iki YENİ RPC'yi kapsıyordu (kısmi sertleştirme bulgusu); (b) §1 seed koşulsuz UPDATE → insert-eksikse: re-apply sahibin UI'dan değiştireceği eşiği EZMEZ — canlı koruma-probu: değer 45'e çekildi → re-apply 45'i KORUDU → 40 geri geldi (protokol_ayar guncellendi izi 22:40:02/22:40:49 UTC). Final SHA `80fbaff2`; db-validate C1 PASS (`reports/db-validation-80fbaff2.md`); DEMO idempotent re-apply: 5/5 fonksiyon proconfig pinli, ACL deseni bozulmadı, çapalar değişmez (sessiz 8 / muayene {180,173,902} / v_eligible toplam 9 — yaş-fallback küçük sessiz_gun'lular listele ≥50 dışı, F9 kupe-31 notuyla tutarlı); `_uret` dry-run esik_gun=40, adet=3, üretilen=0. (c) 3 ön-existing test kırmızısı (bc-tarih takvim ×2 + LUNA-3 canlı-harita drift; haritasız kolonlar 20260923/24 ovsync vaka-kapanış ailesinden — dal diff'inde 0 geçiş) triaja alındı → BUGS.md NOT-TEST-TRIAJ-S2. **Süreç notu:** teyit ajanı verifier-mandasını aşarak ürün dosyasına+dbye dokundu; net durum onarım ajanınca kapılarla yeniden-doğrulandı (bkz. kanıt zarfı §7) |
