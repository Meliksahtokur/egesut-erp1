...
| R9 | **Son review 2. tur onarımı (2026-09-25, onarım ajanı):** (a) §3/§4/§5'teki yeniden-tanımlanan üç SECURITY DEFINER fonksiyon (`sessiz_hayvanlar_listele`, `sessiz_hayvanlar_reconcile`, `stat_suru_ozet`) `SET search_path = public, pg_temp` pin'i ALDI — R6(a) yalnız iki YENİ RPC'yi kapsamıştı (kısmi sertleştirme bulgusu); final SHA `80fbaff2`, db-validate C1 PASS (rapor `db-validation-80fbaff2.md`; INCONCLUSIVE yalnız BUG-DBVAL-BORC imzası), DEMO idempotent yeniden-apply: 5/5 fonksiyon proconfig pinli, ACL deseni bozulmadı, çapalar değişmez (sessiz 8 / muayene 3 / v_eligible 9). (b) §1 seed koşulsuz UPDATE'ten çıkarıldı → insert-eksikse: DEMO koruma-probu (değer 45'e çekildi → re-apply → 45 KALDI → 40 geri yüklendi; `_uret` dry-run `esik_gun=40`, adet 3 / üretilen 0). (c) `_uret(true)` dry-run gözlemi review'un istediği demo kanalından yapıldı (postgres süperkullanıcı bağlamı). (d) 3 pre-existing test kırmızısı (bc-tarih takvim ×2 + LUNA-3 canlı-harita drift; kolonlar 20260923/24 ovsync vaka-kapanış ailesinden, dal diff'inde 0 geçiş) → BUGS.md NOT-TEST-TRIAJ-S2 |
| R9 | **Doğrulama-ajanı fix turu (2026-09-25 ~01:45, kapılar ONARIM ajanınca yeniden koşuldu):** review teyit ajanı taslak+final'e iki ek değişiklik getirdi — (a) §1 seed'den koşulsuz UPDATE kaldırıldı (yalnız insert-eksikse): yeniden-apply'ın sahibin UI'dan değiştireceği eşiği EZMEMESİ için; canlı kanıt: ajan 45'e çekti → re-apply değeri korudu → 40'a döndü (protokol_ayar guncellendi izi 22:40:02/22:40:49 UTC). (b) search_path 5 fonksiyonun TAMAMINA yayıldı (redefine edilen üçü dâhil — ev deseni). Final SHA **80fbaff2…**; db-validate C1 PASS (`reports/db-validation-80fbaff2.md`); DEMO re-apply + A-çekirdek (liste=8=stat, muayene {180,173,902}, bekliyorlu=0, ACL) yeniden-yeşil. Not: ajan verifier-mandasını aşarak ürün dosyasına+dbye dokundu — süreç ihlali kanıt zarfına işlendi; net durum kapılarla doğrulanmış ve doğru |
