...
> **Tarih:** 2026-09-24 · **Branch:** `ovysch-feature-cila-turu` · **Kaynak:** `reports/plans/ovsync-cila-plan-1.md` + `reports/plans/ovsync-cila-sentez.md` (§3 Adım 1, §5A, §9) + canlı kod/demo şema doğrulaması (bu spec yazımında yeniden ölçüldü)
...
| # | Kanıt | Etiket |
|---|---|---|
| K1 | Demo'da 184/199/208: `kisir=true`, Aktif, Dişi, Sağmal (Laktasyonda); 184 ve 199'da `tohumlama_durumu='gebe'` (küçük harf), 208'de NULL | OBSERVED `supabase_query hayvanlar kupe_no=in.(184,199,208)` (demo, 2026-09-24) |
| K2 | Üç hayvanın OVSYNC_BASLAT görevleri KAPALI: `0c698720` (184), `bb1fee13` (199), `aec39359` (208) — `tamamlandi=true, iptal=false, kapatan_ref='case:…'` | OBSERVED `gorev_log hayvan_id=in.(…) gorev_tipi=OVSYNC_BASLAT` (demo, 2026-09-24) |
| K3 | Üç açık senkron vakası: `cases f72f320c-5955…` (184), `37b98c0c-fb4f…` (208), `19febaa1-ff69…` (199) — `status='active', protocol_family='OVSYNC'` | OBSERVED `cases id=in.(…)` (demo, 2026-09-24) |
| K4 | Uygunluk gövdesi `_acik_disi_hedef_ic`'te; `SELECT durum, cinsiyet` — `kisir` sorgulanmıyor; muafiyetler: Aktif/Dişi, son tohumlama Gebe/Bekliyor, aktif protocol_family vakası, açık OVSYNC_BASLAT, kural tabanı | CONFIRMED `supabase/migrations/20260924000002_dryrun_bayrak_bagimsiz.sql:L33-59` + OBSERVED canlı `pg_get_functiondef('_acik_disi_hedef_ic')` birebir aynı (demo, 2026-09-24) |
| K5 | `_acik_disi_ovsync_hedef` artık yalnız bayrak kapısı + delegasyon (`RETURN _acik_disi_hedef_ic(...)`) | CONFIRMED `20260924000002:L74-90` + OBSERVED canlı gövde |
| K6 | `start_first_service_protocol` muafiyet seti: AKTIF_DEGIL / GEBE / BEKLIYOR / AKTIF_SENKRONIZASYON — **KISIR yok**; hayvan okuma `SELECT * INTO v_h FROM hayvanlar … FOR UPDATE` (kisir alanı zaten `v_h`'te taşıyor); muafiyet yolu: görev `iptal=true, tamamlandi=true, kapatan_ref='ILK_TOH_MUAF:<neden>'` + instance `iptal` + `FIRST_SERVICE_SKIPPED` audit + dönüş `{ok:true, atlandi:<neden>}` | CONFIRMED `20260924000001_ovsync_pg_r32_acik_disi.sql:L709-744` (ELSIF zinciri :719-728) + OBSERVED canlı `pg_get_functiondef` birebir aynı (demo, 2026-09-24) |
| K7 | `ilk_tohumlama_zamanlayici`: gerçek tarama dalı `_acik_disi_ovsync_hedef` çağırır (:L246), dry-run dalı `_acik_disi_hedef_ic` çağırır (:L140, :L154); dry-run tarama SELECT'i :L128-134; gerçek-dal `duve_tabansiz` sayacı :L262-277 inline koşullu (kısırdan habersiz) | CONFIRMED `20260924000002:L99-301` |
| K8 | `ovsync_baslat_uyarilari()` dönüş alanları: gorev_id, hayvan_id, kupe_no, kategori, hedef_tarih, hedef_saat, tai_tarihi, kaynak, taban_turu — `kisir` YOK; `hayvanlar h` JOIN'i mevcut (alan ekleme mekanik) | CONFIRMED `20260924000001:L1089-1120` + OBSERVED canlı gövde birebir (demo, 2026-09-24) |
...
| K12 | Demo'da AÇIK OVSYNC_BASLAT görevi sahiplerinin (31 görev) **hiçbirinde** `kisir=true` yok (31/31 `kisir=false`); 188 (`f5124a14-…`) açık OVSYNC_BASLAT hedef 2026-10-06, `kisir=false` — sahibin "çift zincir KASITLI" kararının canlı çapası | OBSERVED `gorev_log gorev_tipi=OVSYNC_BASLAT&tamamlandi=false&iptal=false` + `hayvanlar` embed (demo, 2026-09-24) |
| K13 | Üretim kancaları uygunluk fonksiyonunu canlıda çağırıyor: `hayvan_ekle` (overload'lardan biri) ve `tohumlama_sonuc_bos` gövdeleri `_acik_disi%` içeriyor | OBSERVED `pg_proc pg_get_functiondef LIKE '%_acik_disi%'` taraması (demo, 2026-09-24) |
| K14 | Kısır guard'ı öncülleri canlıda mevcut (desen tekrarı değil, aynı ailenin genişletmesi): `hayvan_tohumlanabilir_onayla` → `RAISE 'Kısır hayvan tohumlanamaz'`; `hayvan_guncelle` overload'ından biri kısır validation'ı taşıyor; `_yeniDogumGun` kısırı UI'da atlıyor | OBSERVED canlı `pg_get_functiondef` + CONFIRMED `js/ui.js:L1551-1552`, ground_truth `hayvan_guncelle` kısır bloğu (canlı overload teyitli) |
...
| 2 | `supabase/tests/ovsync_pg_kabul.sql` | **APPEND** (yalnız ekleme; ÖZET bölümünden ÖNCE `R32-11` blokları) | §8 T1-T7 |
| 3 | `js/ui.js` | **İKİ LOKAL NOKTA**: `_ovsyncBaslatBtnHtml` (:L1162-1166) ve `_ovSatir` (:L1834-1844, yalnız buton hücresi :L1840-1843) | §7 |
| 4 | `index.html` | **TEK SATIR**: `js/ui.js?v=20260924-01` → `?v=20260925-01` (tek-değer damga kuralı) | :L2336 |
...
- Hayvan kartı (:L1597 `_animalCardHtml`) dokunulmaz (U3 — §4.5).
- Damga: `index.html:L2336` `?v=20260925-01`.

## 8. Kabul testi maddeleri (ölçülebilir)

**Ortam notu:** T1-T7 izole ortamda koşulur (`supabase/tests/ovsync_pg_kabul.sql` sözleşmesi: sarmalayıcı `BEGIN…ROLLBACK`, `kb_ok` OZET sayacı — K15). Demo-apply sonrası T8-T10 demo üzerinde salt-okunur koşar (demo yazma sahibin onayıyla serbest; yine de test betiği varsayılan olarak izole baseline'da koşar). T11-T13 sahip yürüyüşüdür (browser testi ajan koşmaz — plan bandı).

**DB — `R32-11` kabul blokları (tests/ovsync_pg_kabul.sql'e APPEND, ÖZET'ten önce):**
...
- [ ] **T3 (fail-closed DEĞİL):** T2'deki hayvan `UPDATE … SET kisir=false` → taban var (kb_hayvan 1500 günlük) ve zemin temizse `_acik_disi_gorev_kur(id)` IS NOT NULL (görev açılır). — D4
...
- [ ] **T9:** Açık OVSYNC_BASLAT sahiplerinde `kisir=true` sayısı = 0 (K12'nin migration sonrası tekrarı); `ovsync_baslat_uyarilari()` çıktısında tüm `uyarilar[].kisir=false`.
...
3. **DB rollback (tek psql script, tek transaction):** dört gövdenin kisir-ÖNCESİ halleri `CREATE OR REPLACE` ile geri yazılır:
   ...
   - `start_first_service_protocol` → `20260924000001:L662-846` gövdesi (KISIR ELSIF'i çıkarılmış mevcut zincir);
...
| V6 | `cases`/`gorev_log` yazan üçüncü bir kısır-yolu yok (ör. başka RPC kısır hayvana ovsync vakası açmıyor): repo-grepti `_vaka_ac_tek` çağrıları yalnız `start_first_service_protocol` + vaka-açılış UI yolu; vaka-açılış UI yolu manuel-olduğundan blok dışı (kullanıcı hayvanı elle vaka-ekle'den kurtaramaz varsayımı YOK — bu bilinçli kapsam kararı, manuel vaka açma operatör eylemidir) | INFERRED + kapsam kararı |
...
1. **Kısır temizliği (sahibin "evet" kararı, sentez §9 S1):** üç AÇIK senkron vakası `f72f320c` (184), `37b98c0c` (208), `19febaa1` (199) + bağlı "Gun 1" TEDAVI_GUN/SEANS görevleri — Tur 2 **Adım 3**'te dry-run liste onaylı koşum (sentez §3). Bu migration temizliği BEKLEMEZ: M1 kısır kontrolünü aktif-vaka kontrolünden ÖNCE koyduğu için açık vakalı kısır hayvan da üretim dışında kalır. Temizlik ayrı operasyon-spec (Adım 3 zarfı); silme değil `iptal`/`closed` işaretlemesi.
2. **188 çift zincir KASITLI (sahip kararı):** S1 yalnız OVSYNC_BASLAT üretim/başlatma/UI'ya dokunur; ILAC/PG zincirlerine ve 188'in açık OVSYNC_BASLAT'ına (`f5124a14…`, hedef 2026-10-06) dokunmaz (K12).
...
*Spec sonu. Yazım yetkisi yalnız `docs/plans/2026-09-24-ovsync-cila/` ile sınırlıydı; koda dokunulmadı. Kanıtlar 2026-09-24 tarihinde canlı repo (HEAD: bacf648 üstü çalışma kopyası) + demo DB salt-okunur sorgularıyla toplandı.*
