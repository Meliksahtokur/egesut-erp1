...
## DeepSeek TUI Adaptation

| Pattern in this skill | DeepSeek TUI equivalent |
|----------------------|------------------------|
| `Read` file | `read_file` tool |
| `Edit`/`Write` file | `edit_file` / `write_file` / `apply_patch` tools |
| `Bash` command | `exec_shell` tool |
| `Grep` search | `grep_files` tool |
| `TodoWrite` / checklist | `checklist_write` / `checklist_update` tools |
| Launch agent (parallel) | `agent_open` (parallel in one turn) |
| Agent result | `agent_eval` to fetch; verify side effects with `read_file` |
| `WebSearch` | `web_search` tool |
| `WebFetch` | `fetch_url` tool |
| `$ARGUMENTS` | Prompt passed directly as user message |
| `test` / `run tests` | `run_tests` (cargo) or `exec_shell` |
| GitHub read/write | `github_issue_context`, `github_comment`, etc. |
| MCP tools | `mcp_tools--bank_*` prefix for tools-bank MCP |

### Notlar

- Bu skill genel workflow tanımlar — Claude'a özel tool adı kullanmaz, doğrudan çalışır.
- Agent çağrıları için `agent_open` + `agent_eval` kullanılır.
- Sub-agent sonuçları self-report'tur; dosya değişikliği varsa `read_file` ile doğrula.
- Plan takibi için `checklist_write` tercih edilir.
- Test için bu projede `run_tests` yerine `exec_shell` ile `npx playwright test` kullanılır.
