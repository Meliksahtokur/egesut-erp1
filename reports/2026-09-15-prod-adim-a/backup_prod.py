#!/usr/bin/env python3
"""P2 Adım A — PROD mantıksal yedek (Mgmt API, yazma YOK).

public şemasındaki her tablonun tüm satırları (json_agg, parçalı) + fonksiyon/
trigger/policy/grant/index/kolon/constraint/view tanımları → tablo başına bir
JSON dosyası, REPO DIŞI hedefe. manifest.json: satır sayıları + sha256 + süre.
...
OUT = pathlib.Path("/home/melik/tmp/agents/prod-yedek-2026-09-15")
...
# ---- 1. public tabloları + kesin satır sayıları ----
tables = qi("SELECT c.relname AS name, c.relrowsecurity AS rls "
            "FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace "
            "WHERE n.nspname='public' AND c.relkind IN ('r','p') ORDER BY 1;")
counts = qi("SELECT relname, n_live_tup FROM pg_stat_user_tables WHERE schemaname='public' ORDER BY 1;")
live = {r["relname"]: r["n_live_tup"] for r in counts} if isinstance(counts, list) else {}
tab_meta = {}
for t in tables:
...
for name, m in sorted(tab_meta.items()):
    ...
    while True:
        ...
        if err or not isinstance(data, list) or not data:
            if chunk > 50:
                chunk = max(chunk // 2, 50)   # paket çok büyüdü/hata: küçült, aynı offset'i dene
                continue
...
# ---- 4. manifest: satır doğrulaması + sha256 ----
verify = {}
for t, exp in EXPECTED.items():
    got = tab_meta.get(t, {}).get("rows")
    verify[t] = {"expected_P1": exp, "backup": got, "match": got == exp}
meta["verify_vs_P1"] = verify
...
print("\nDoğrulama (P1 referansı ile):")
bad = []
for t, v in verify.items():
    mark = "OK " if v["match"] else "FARK"
    print(f"  [{mark}] {t}: beklenen={v['expected_P1']} yedek={v['backup']}")
    if not v["match"]:
        bad.append(t)
print(f"meta dosya sayısı: {len(meta['files'])}, süre: {meta['duration_s']}s")
if bad:
    print(f"EŞLEŞMEYEN: {bad} → DUR (zarf gereği)")
    sys.exit(2)
print("TÜM REFERANS SAYILARI EŞLEŞTİ")
